...
if {[catch {
    ...
    if {[catch {
	set neorv32_pkg [lsearch -inline $vhdl_filenames "*/neorv32_package.vhd"]
	set top_file    [lsearch -inline $vhdl_filenames "*/$top_level_file"]
	
	set vhdl_without_pkg [lsearch -all -inline -not $vhdl_filenames "*/neorv32_package.vhd"]
	set neorv32_files    [lsearch -all -inline -not $vhdl_without_pkg "*/$top_level_file"]

	read_vhdl -library neorv32 $neorv32_pkg
	read_vhdl -library neorv32 $neorv32_files
	read_vhdl $top_file
    } errmsg]} {
        if {$vhdl_filenames == ""} {
