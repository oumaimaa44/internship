 ...
 */
 ...
 /*
 *                  Dhrystone benchmark for the neorv32
 *                  Edited Juin 02, 2026
 */
...
Rec_Pointer     Ptr_Glob,
                Next_Ptr_Glob;
/*  before : malloc() gives memory, if malloc fails -> crashes
  After : Rec_Glob_1 and Rec_Glob_2 always exists, no malloc needed */
#ifdef NEORV32
static Rec_Type Rec_Glob_1;
static Rec_Type Rec_Glob_2;
#endif
...
{
  (void)unused;
  return (long)neorv32_cpu_get_cycle();
}

long insn(long *unused)
{
  (void)unused;
  return (long)neorv32_cpu_csr_read(CSR_MINSTRET);
}
#endif
  ...
  /* Main and Proc_0 in the Ada version             */
{
  ...
  /* if compiling with neorv32 : don't use malloc, use the two static records
    else : keep the original Dhrystone malloc behavior */
  #ifdef NEORV32
