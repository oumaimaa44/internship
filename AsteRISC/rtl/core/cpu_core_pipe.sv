...
#(
  ...
  //! front-end prediction: a branch target buffer and a return address stack
  //! (see `cpu_btb` and `cpu_ras`). They predict from a *fetch address*, before
  //! the instruction has come back from memory, which is what makes a correctly
  //! predicted transfer cost nothing at all -- where `p_branch_pred` can at
  //! best redirect the front-end once the instruction has been decoded, and
  //! therefore always pays the barriers in between. The two are orthogonal and
  //! compose: what the buffer misses, the decode-slot predictor still catches
  //! one slot later.
  parameter p_btb_entries  = 0,           //! branch target buffer entries (0 = none)
  parameter p_btb_ways     = 1,           //! branch target buffer associativity (1, 2 or 4)
  ...
  *******************************************************/
  ...
  logic         issue_EX;                 //! present the data access on the data bus
  logic         dmem_wr_req;              //! a store is presented on the data bus
  logic         dmem_rd_req;              //! a load is presented on the data bus
  logic         dbus_stall;               //! the data bus refuses the access presented
  logic         en_IF;
  logic         en_IC;
  logic         en_ID;
  logic         en_RF;
  ...
  logic         fetch_valid;              //! the fetch stage holds a word not yet taken

  logic [31: 0] pc_F;                     //! program counter (raw fetch output)
  logic [31: 0] pc_inc_F;                 //! program counter +4 (raw fetch output)
  logic         pred_F;                  //! the front-end left the sequential path after this word
  logic [31: 0] pred_pc_F;               //! ...and this is the address it went to
  ...
  logic         pred_IF;                 //! the front-end left the sequential path after this word
  logic [31: 0] pred_pc_IF;              //! ...and this is the address it went to
  ...
  logic         pred_IC;                 //! the front-end left the sequential path after this word
  logic [31: 0] pred_pc_IC;              //! ...and this is the address it went to
  ...
  *******************************************************/
  ...
  //! whether a front-end predictor is built at all. Every expression this
  //! selects has the pre-buffer form on its other side, so a core without a
  //! buffer elaborates to exactly the design that existed before there was one.
  localparam int BTB_ON = (p_btb_entries != 0) ? 1 : 0;
  ...
  wire          fetch_predicted;          //! the front-end left the sequential path after `instr`
  wire  [31: 0] fetch_predicted_pc;       //! ...and this is the address it went to
  wire          ic_override;              //! the IC slot must send the front-end elsewhere
  wire          rf_override;              //! the front-end is not where the RF slot instruction goes
  logic         bp_upd_valid_fe;          //! a control transfer resolved, teach the front-end
  logic [31: 0] bp_upd_pc_fe;             //! ...its address
  logic [31: 0] bp_upd_target_fe;         //! ...the target it went to
  bp_kind_e     bp_upd_kind_fe;           //! ...what it was
  logic         bp_upd_taken_fe;          //! ...and whether it was taken
  ...
  *******************************************************/
  ...
  cpu_hazard #(
    ...
    .i_ic_override      ( ic_override           ),
    .i_rf_override      ( rf_override           ),
  ...
  //! A redirection that has resolved but has not reached the fetch stage yet.
  //!
  //! `p_redirect_buf` is exactly that one cycle of delay, so with the register
  //! the fetch has to be told the front-end went wrong before it is given the
  //! target; without it, the two arrive together and there is nothing to
  //! announce in advance.
  wire          redirect_deferred = (p_redirect_buf != 0) & redirect;
  ...
  cpu_fetch_pipe #(
    ...
    .ibus_ack       ( ibus_ack              ),
    .i_en           ( en_IF                 ),
    .i_redirect_req ( redirect_deferred     ),
    .i_redirect     ( redirect_fetch        ),
    .i_redirect_pc  ( redirect_fetch_pc     ),
    .i_upd_valid    ( bp_upd_valid_fe       ),
    .i_upd_pc       ( bp_upd_pc_fe          ),
    .i_upd_target   ( bp_upd_target_fe      ),
    .i_upd_kind     ( bp_upd_kind_fe        ),
    .i_upd_taken    ( bp_upd_taken_fe       ),
    .o_predicted    ( fetch_predicted       ),
    .o_predicted_pc ( fetch_predicted_pc    ),
    .o_pc           ( pc                    ),
    .o_pc_inc       ( pc_inc                ),
    .o_instr        ( instr                 ),
    .o_valid        ( fetch_valid           )
  ...
  *******************************************************/
  ...
  wire  [31: 0] redirect_pc_taken = (sel_pc == pc_alu) ? (alu_out & 32'hfffffffe)
                                                       : (pc_RF + $signed(imm_RF));
  wire  [31: 0] redirect_pc_RF = (((p_branch_pred != 0) | (BTB_ON != 0)) & ~branch_taken)
                                 ? pc_inc_RF : redirect_pc_taken;
  wire  [31: 0] redirect_pc_IC = pc_IC + $signed(imm);

  /*******************************************************
    Front-end prediction: checking it (`p_btb_entries`)

    A buffer predicts from an address, before the instruction at that address
    has been read, let alone decoded. So nothing it says can be trusted, and
    everything it says has to be checked -- not only the direction of a branch,
    as a decode-slot predictor requires, but the target too, and even whether
    the address was a control transfer at all.

    One comparison answers all of it. The front-end went somewhere; the
    instruction, once resolved, goes somewhere. If the two addresses differ the
    front-end is repaired, by the same redirection path that has always
    repaired it, and if they agree nothing happens at all -- which is the whole
    point: a correctly predicted transfer costs no bubble anywhere.

    The comparison is made twice, in the two slots that can already redirect:

      * at the IC slot, where the decoder knows the target of everything but an
        indirect jump. It catches a buffer hit on an address that is not a
        branch, and a `jal` or a predicted branch whose target the buffer had
        wrong, one slot earlier than the execute unit would.
      * at the RF slot, where the execute unit resolves everything else, an
        indirect jump included.
  *******************************************************/

  //! where the front-end actually went after the IC slot instruction
  wire  [31: 0] frontend_pc_IC = pred_IC ? pred_pc_IC : pc_inc_IC;

  //! where the IC slot believes it should have gone
  wire  [31: 0] ic_want_pc = (((p_early_jal != 0) & (sel_br == br_jal)) | predict_taken)
                           ? redirect_pc_IC : pc_inc_IC;

  //! Which instructions the IC slot is entitled to decide the next address of.
  //! Getting this wrong does not produce a wrong result -- the RF slot repairs
  //! whatever this slot does -- it produces a slot that overrules the buffer on
  //! instructions it knows nothing about, and cancels the predictions that were
  //! supposed to be free:
  //!
  //!  * an indirect jump: its target needs a register value, so this slot never
  //!    knows it, and a return predicted off the stack must be left alone here.
  //!  * a conditionnal branch the buffer already predicted: two direction
  //!    predictors would otherwise fight over it every cycle, and the buffer is
  //!    the one whose answer is already free. What it gets wrong the execute
  //!    unit repairs, at the price a misprediction always costs.
  //!  * a conditionnal branch when there is no decode-slot predictor at all:
  //!    with `p_branch_pred = 0` nothing here has an opinion on its direction,
  //!    and assuming "not taken" would undo every prediction the buffer made.
  wire          ic_owns = ~jalr_instr_bp
                        & ~(cond_br_bp & (pred_IC | (p_branch_pred == 0)));

  assign ic_override = (BTB_ON != 0) & ic_owns & (ic_want_pc != frontend_pc_IC);

  //! where the front-end actually went after the RF slot instruction, and where
  //! that instruction really goes now that the execute unit has resolved it
  wire  [31: 0] frontend_pc_RF = predicted_RF ? ctrl_RF.pred_pc : pc_inc_RF;
  wire  [31: 0] correct_pc_RF  = branch_taken ? redirect_pc_taken : pc_inc_RF;

  assign rf_override = (BTB_ON != 0) & (frontend_pc_RF != correct_pc_RF);

  /*******************************************************
    Front-end prediction: teaching it

    The buffer learns from the RF slot, where the verdict is produced, and only
    from an instruction that actually retires from it -- `en_RF` is the cycle it
    moves on, so each transfer teaches the buffer exactly once, however long the
    slot was frozen.

    What it learns is a *kind*, not an opcode: the buffer only ever has to
    distinguish a target it can remember from one it cannot, and a call from a
    return. That distinction is the RISC-V calling convention, which says a
    `jalr` linking into `x1` or `x5` is a call and a `jalr` returning through
    one of them is a return.
  *******************************************************/

  wire  [4: 0]  upd_rd  = ctrl_RF.wbc.wb_addr;
  wire  [4: 0]  upd_rs1 = ctrl_RF.rs.a.rd1_addr;
  wire          upd_link_rd  = (upd_rd  == 5'd1) | (upd_rd  == 5'd5);
  wire          upd_link_rs1 = (upd_rs1 == 5'd1) | (upd_rs1 == 5'd5);

  always_comb begin: front_end_update
    bp_upd_pc_fe     = pc_RF;
    //! a branch that resolves not taken still teaches the buffer where it
    //! *would* have gone: that is what makes it predictable the next time round
    bp_upd_target_fe = redirect_pc_taken;
    bp_upd_taken_fe  = branch_taken;
    if (ctrl_RF.cond_branch) begin
      bp_upd_kind_fe = bp_branch;
    end else if (upd_link_rd) begin
      //! `jal`/`jalr` writing a link register: a call, whichever it is
      bp_upd_kind_fe = bp_call;
    end else if ((sel_br_RF == br_jalr) && upd_link_rs1) begin
      bp_upd_kind_fe = bp_ret;
    end else begin
      bp_upd_kind_fe = bp_jump;
    end
    bp_upd_valid_fe  = (BTB_ON != 0) & valid_RF & en_RF
                     & (ctrl_RF.cond_branch | (sel_br_RF == br_jal) | (sel_br_RF == br_jalr));
  end
  ...
  //! them there.
  wire          mispredict_RF_c = (BTB_ON != 0)        ? rf_override
                                : (p_branch_pred != 0) ? (branch_taken ^ predicted_RF)
                                                       :  branch_taken;
  wire          redirect_req_RF = mispredict_RF_c
                                & ~((BTB_ON == 0) & (p_early_jal != 0)
                                    & (sel_br_RF == br_jal));
  ...
  /*******************************************************
    ...
    The fetch stage reports its output as invalid while it holds nothing -- the
    bus has not answered yet -- or while what it holds was fetched from a path a
    redirection has abandoned. Either way it arrives here as a bubble, so
  ...
  *******************************************************/
  ...
  always_comb begin
    ...
    pc_inc_F = pc_inc;
    valid_F  = fetch_valid;
    pred_F    = fetch_predicted;
    pred_pc_F = fetch_predicted_pc;
  ...
  generate
    if (N_IF == 0) begin : g_barrier_IF
      ...
      for (genvar k = 0; k < N_IF; k++) begin : g_lvl
        always_ff @(posedge i_clk) begin
          if (i_rst) begin
            ...
            pd_c[k]      <= (k == 0) ? pred_F    : pd_c[k-1];
            pp_c[k]      <= (k == 0) ? pred_pc_F : pp_c[k-1];
          end
        end
      end
      ...
      assign pred_IF    = pd_c[N_IF-1];
      assign pred_pc_IF = pp_c[N_IF-1];
    end
  endgenerate
  ...
  generate
    if (p_stage_IC == 0) begin : g_barrier_IC
      ...
      for (genvar k = 0; k < p_stage_IC; k++) begin : g_lvl
        always_ff @(posedge i_clk) begin
          if (i_rst) begin
            ...
            pd_c[k]      <= (k == 0) ? pred_IF      : pd_c[k-1];
            pp_c[k]      <= (k == 0) ? pred_pc_IF   : pp_c[k-1];
          end
        end
      end
      ...
      assign pred_IC         = pd_c[p_stage_IC-1];
      assign pred_pc_IC      = pp_c[p_stage_IC-1];
    end
  endgenerate
  ...
  *******************************************************/
  ...
  always_comb begin
    ...
    //! Where the front-end went after this instruction, as the ID barrier
    //! latches it. With a buffer that is either what the buffer decided a cycle
    //! before the instruction arrived, or -- when this very slot is redirecting
    //! it -- the target it is being sent to; whichever it is, the resolving
    //! slot has to compare against the address the front-end really followed.
    ctrl_ID_n.predicted        = (BTB_ON != 0) ? (redirect_early | pred_IC)
                                               : predict_IC;
    ctrl_ID_n.pred_pc          = 32'd0;
    if (BTB_ON != 0) begin
      ctrl_ID_n.pred_pc        = redirect_early ? redirect_pc_IC : pred_pc_IC;
    end
  ...
  *******************************************************/
  ...
  //! The data access presented on the bus.
  //!
  //! It is qualified by `issue_EX` rather than by `en_EX`: `en_EX` carries the
  //! data bus stall, so building the request from it would make the request
  //! depend on the `busy` it produces. `issue_EX` is the same enable with that
  //! one term removed, which is exactly what the bus protocol needs -- the
  //! request stays presented, unchanged, for as long as the bus refuses it,
  //! because the EX slot is frozen while it does.
  assign dmem_wr_req = en_dmem_wr_EX & valid_EX & issue_EX;
  assign dmem_rd_req = en_dmem_rd_EX & valid_EX & issue_EX;

  //! the EX slot cannot move on while the bus refuses the access it presents
  assign dbus_stall  = (dmem_wr_req | dmem_rd_req) & dbus_busy;
  ...
  cpu_mem #(
    ...
    .i_en_dmem_wr   ( dmem_wr_req           ),
    .i_rf_rd2_data  ( rf_rd2_data_EX        ),
    .i_en_dmem_rd   ( dmem_rd_req           ),
