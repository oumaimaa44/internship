...
package pck_control;
  ...
  //! What a front-end predictor believes the instruction at a given address is.
  typedef enum logic [1:0] {
    bp_branch,                    //! conditionnal branch: direction must be predicted
    bp_jump,                      //! unconditionnal, target known from the instruction
    bp_call,                      //! a call: pushes a return address on the stack
    bp_ret                        //! a return: its target is popped off the stack
  } bp_kind_e;
/*
  typedef struct {
