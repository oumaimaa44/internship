...
package pck_pipe;
  ...
  *****************************************************/
  ...
  typedef struct packed {
    ...
    //! where the front-end actually went after this instruction, when it left the sequential path.
    logic [31: 0] pred_pc;                //! front-end target (ID only)
    //! index the dynamic predictor used for that prediction, carried down to the slot that resolves the branch
