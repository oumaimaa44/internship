...
#(
  ...
  //!
  //! `rd_data` and `ack` belong to the cycle after the request -- the memories
  //! answer with a registered output -- so they are selected by the peripheral
  //! that *was* addressed.
  //!
  //! `busy` is not selected at all: it is the or of every peripheral, because
  //! what it has to say is "a transfer is still outstanding somewhere", not
  //! "the peripheral addressed this cycle is not ready". The two differ because
  //! the core does not hold its request for the whole transfer -- `cpu_fsm`
  //! asserts `o_en_dmem_wr` in `st_execute` and then waits in `st_memory` with
  //! nothing on the bus -- so a store in flight addresses no peripheral at all
  //! and any selected `busy` would read as free, letting the core walk on and
  //! the next access overwrite the one still in flight.
  //!
  //! Only the data memory branch can ever raise `busy` in this SoC, so the or
  //! is exactly the same signal as a correct selection would give, with none of
  //! the ways of getting the selection wrong. As long as no slave stalled at
  //! all, none of this showed.
  ...
  always_comb begin: bus_busy
    dbus_busy = 1'b0;
    for (integer i = 0 ; i < p_num_masters ; i++) begin
      dbus_busy = dbus_busy | i_busy[i];
    end
  end
